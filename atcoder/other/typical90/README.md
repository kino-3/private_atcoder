...
- `pass` と書かれている問題は, 保留にしている問題。
- `PASS` と書かれている問題は, 解説を見て解いた問題。
...
- 5: 平方分割
- 10: dp だと O(N^2)
- 20: ∫1/x は O(logN) なので, combination 使えば O(N(logN)^2) になる。
- 55: ライブラリ整理 + 実装

ほぼ同解法。自解法は選ぶものをoとして, その個数を固定したとき, [oxx][oxx][oxx][o]{x}{x}{x}{x}{x} の [] と {} の並び替えに帰着される。
...
- 10: 貪欲法で行けるかと思ったが無理だった
- 18: 蟻本の囚人にわいろを渡す問題に似ていることに気付く -> 再帰
- 33: 実装

同解法。区間DPはO(N^2)~O(N^3)となることが多いらしい。
...
- 長さ N の 0 と 1 からなる文字列は, その両端に 0 を付けて(これを S とする), `S[i] == S[i+1]` かどうかを TF で表すと, 長さ N+1 の T と F からなる文字列 S' (ただし, T は偶数個) と相互変換できる。(どちらも 2^N の状態をとりうる)。
- S の [L,R] の範囲の 0/1 反転は, S' の L と R+1 の T/F 反転に相当する
- このとき, L, R+1 に辺を張ったグラフとして考えると, 複数の範囲からなるグラフが連結の時, S はすべての状態をとりうる。
...
同解法。結局原因がよくわからなかった。nCk で n < k となっていたから ?
...
## 054: 最短経路問題 (部分集合が全結合の場合)

- 3: 最短経路問題
- 6: E,Vともに大きすぎて普通にグラフを作るとサイズが大きすぎる
- 15: おそらく典型解法があるので PASS

※ 全結合された部分集合に共通ノード(ハブ)を新規作成して, その共通ノードと集合のノードを半分のコストの辺で結ぶ。この方法ははじめて知った。
...
## 057: XOR の問題を mod 2 の行列の掃き出し法で解く
...
- 25: 制約条件を見間違えていたことに気付く
- 47: 実装
- 52: デバッグ (dfs の後処理を忘れていた)

同解法。4*4だと探索数の上限は30000弱らしい。

## 073: 木DP

- 3: 木DP?
- 25: 木DP と約数の総和のような考え方でいける
...
桁数が少ない時で実験すると答が推測できる。
不変量を考える。(操作によって変わらない, 必ず減少する, ... など)
...
20 個の条件を全て満たす場合の数を考えるのは難しいが, 条件をすべて満たさない場合の数を考えるのは簡単なとき -> 包除原理
...
よって, 0, 2 個の時はプラスで, 1, 3 個のときはマイナス

※ ビット演算の時は, (1LL << N) とする必要がある。Q051 のエラーもこれか?

## 081: 二次元累積和

- 2: 分割統治法?
- 6: 領域が小さいので二次元累積和で間に合いそう
- 20: 実装
- 27: デバッグ (KがK-1になっていた)
...
同解法。偶奇の場合分けのところは逆元でもよい。

## 083 TLE するケースを考える/平方分割(閾値)/クエリ平方分割

- 2: テストセット1はO(NQ)で通る
- 13: 実装
- 30: 間に合う気がしないので, テストセット2は PASS

TLE するケースを考えるまでは思いついた。

- 解法1. 平方分割(閾値):
    - 以下により, 各クエリの計算量が O(√M) となるので, 全体計算量は O(Q√M)
        - edge が √M 以上のノードのみ, 正確な色を毎クエリ常に更新する O(√M)
        - key: クエリで指定された node, value: (クエリで指定された color, クエリで指定された最新時刻) を記録する O(1)
        - 出力する色:
            - edge が √M 以上のノード: 正確な色を毎クエリ常に更新されているので, その色を出力する O(1)
            - edge が √M 未満のノード: 隣接する全ノードにおける (クエリで指定された color, クエリで指定された最新時刻) を一覧して, 最新時刻が最新の色を出力する O(√M)
- 解法2. クエリ平方分割
    - 一般に, データ構造をうまく扱えないクエリ処理問題を, クエリを以下の手順で √Q 個ごとに処理することで高速化する。
        - 1. √Q 個のクエリを実行する。この間, √Q 個のクエリ出力が正しくなることだけを考慮して, 保持するデータ全体の正しさは保証しない
        - 2. √Q 個のクエリを一気に実行したとして, 保持するデータ全体を正しく更新する
        - 3. 1, 2 を交互にそれぞれ √Q 回行う
    - 本問の場合は, 以下に相当する。よって, O((Q+M)√Q)
        - 1. √Q 個のクエリで色を出力するノードは高々 √Q 個なので, これらのノードの色だけ毎クエリ常に更新する。O(√Qx√Q) = O(Q)
        - 2. それぞれのノードに対して, 隣接ノードのうち最も最近指定されたノードのクエリの色とする。for node for 隣接edge の計算量は O(M) (O(N^2)ではない)
...
類似解法。約数の計算は 1 回でよかった(自解法は O(logK) 回)。
...
- 6: ワーシャルフロイド + 答で二分探索
- 38: 実装
- 43: デバッグ (ワーシャルフロイドのライブラリが有向グラフ用だった)

ほぼ同解法。`f(x)==k` となる x の個数は, `argmin(f(x)<k) - argmin(f(x)<=k)` である。

## 088: 鳩ノ巣原理

- 20: 同じカードは選ばないとしてよい, max は 4444 -> dp? PASS

条件を満たすような取り方を1つ書き出すコストが小さく, とりうる結果が少ない有限個(8888 通り)しかないとき, 8889 回目の書き出しまでの間に同じ結果になるものが存在する。
なお,「ランダムに生成した結果が条件を満たすとき, ...」とする乱択アルゴリズムを用いると, 今回は条件を満たす確率が低いので TLE する -> DFS + 枝刈り で条件を満たす満たす場合を生成する。
