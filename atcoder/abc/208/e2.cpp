...
map<ll, ll> prod_zfill(ll num) {
    // 000 - num
    map<ll, ll> res;
    ll LIMIT = 10000;
    if (num < LIMIT) {  // num = 000 - 099
        res[0] = num + 1;
        return res;
    }
    res[0] = LIMIT;
...
auto prod_normal_full = prod_normal(999);
auto prod_zfill_full = prod_normal(999);

map<ll, ll> prod_large_normal(ll num) {
    // 1 - num
    ll MILLION = 10000;
    if (N <= MILLION) {
        ll ans = 0;
...
int main() {
    ...
    print_map(prod_normal_full);
    print_map(prod_zfill_full);
    // print_map(prod_normal(12));
    // print_map(prod_zfill(112));
