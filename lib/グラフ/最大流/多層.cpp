...
int main() {
    ...
    //     B[i]--;
    // }
    MaxFlowML max_flow = MaxFlowML({2, 3, 3, 4});
    REP(i, 2) max_flow.add_edge_from_source({0, i});
    max_flow.add_edge({0, 0}, {1, 0});
    max_flow.add_edge({0, 0}, {1, 1});
    max_flow.add_edge({0, 1}, {1, 1});
    max_flow.add_edge({0, 0}, {1, 2});
    REP(i, 3) max_flow.add_edge({1, i}, {2, i});
    max_flow.add_edge({2, 0}, {3, 2});
    max_flow.add_edge({2, 1}, {3, 2});
    max_flow.add_edge({2, 1}, {3, 3});
    max_flow.add_edge({2, 2}, {3, 1});
    max_flow.add_edge({2, 2}, {3, 2});
    max_flow.add_edge({2, 2}, {3, 3});
    REP(i, 4) max_flow.add_edge_to_sink({3, i});
